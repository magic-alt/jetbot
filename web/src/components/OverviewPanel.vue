...
function findMetric(name: RegExp): MetricItem | null {
  ...
  for (const list of Object.values(props.statements)) {
    ...
    for (const m of list) {
      if (name.test(m.name) || name.test(metricKey(m.name))) return m
    }
  }
  return null
}

const revenue = computed(() => findMetric(/^(revenue|net sales|total net sales|total revenue|营业收入|主营业务收入)/i))
const netIncome = computed(() => findMetric(/^(net income|净利润)/i))
const assets = computed(() => findMetric(/^(total assets|资产总计)/i))
const cash = computed(() => findMetric(/(operating cf|operating cash flow|cash generated by operating activities|经营活动产生的现金流)/i))
